...
def preprocess_qwen_vla(sources, tokenizer: transformers.PreTrainedTokenizer, action_tokenizer: ActionTokenizer, has_image: bool = False, has_embody: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    # roles = {"human": "<|im_start|>user", "gpt": "<|im_start|>assistant"}
    ...
    image_token_index = tokenizer.convert_tokens_to_ids("<image>")
    # print(f"image_token_index为: {image_token_index}")
    im_start, im_end = tokenizer.additional_special_tokens_ids
    # unmask_tokens = ["<|im_start|>", "<|im_start|>", "\n"]
    unmask_tokens_idx =  [198, im_start, im_end]
    nl_tokens = tokenizer("\n").input_ids
    ...
    for i, source in enumerate(sources):
        ...
        for conv in source:
            ...
            if conv["from"] == "gpt":
                # print("gpt答案转换")
                if has_embody:
                    real_action_token, sent_value = action_to_lang(conv["value"], action_tokenizer)
                else:
                    sent_value = conv["value"]
            else:
                # print("人类问题转换")
                if has_embody:
                    sent_value = conv["value"]
            ...
            if role in ["user", "system"]:
                ...
                target += encode_id
            # 🧩 调试：查看 token id 和字符串对应关系
            # tokens = tokenizer.convert_ids_to_tokens(encode_id)
            # decoded = tokenizer.decode(encode_id, skip_special_tokens=False)
            # print("\n🔍 当前对话轮角色:", role)
            # print("📥 原始内容:", content)
            # print("🧾 Token IDs:", encode_id)
            # print("🔡 Tokens:", tokens)
            # print("📜 解码结果:", decoded)
            # print("🔗 TokenID-Token 对:", list(zip(encode_id, tokens)))
        ...
        # print(f"before process input_id为: {input_id}")
        # print(f"before process target为: {target}")
        index_198 = np.where(np.array(input_id) == 198)[0]
        # assert index_198[-1]-index_198[-2]==37,f"{index_198[-1]-index_198[-2]}"
        # if index_198[-1]-index_198[-2]!=37:
            # print(f"index_198为: {index_198}")
            # print(f"input_id为: {input_id}")
            # print(f"target为: {target}")
        # print("index_198[-1]-index_198[-2]:",index_198[-1]-index_198[-2])
        # print(f"index_198为: {index_198}")
        input_id[index_198[-2]+1:index_198[-1]-1] = real_action_token
        target[index_198[-2]+1:index_198[-1]-1] = real_action_token
        input_id[index_198[-4]+1:index_198[-3]-1] = real_obs_token
        target[index_198[-4]+1:index_198[-3]-1]=[IGNORE_INDEX] * len(real_obs_token)
        # assert len(input_id) == len(target), f"{len(input_id)} != {len(target)}"
        # len_input_id_after = len(input_id)
        # len_target_after = len(target)
        # if len_input_id_after != len_input_id or len_target_after != len_target:
        #     print(f"before process input_id为: {len_input_id}")
        #     print(f"before process target为: {len_target}")
        #     print(f"after process input_id为: {len_input_id_after}")
        #     print(f"after process target为: {len_target_after}")
        #     print(f"input_id为: {input_id}")
        #     print(f"target为: {target}")
        # print(f"after process input_id为: {input_id}")
        # if(len(target)==140):
        #     print(f"after process target为: {target}")
        ####cjy
        input_ids.append(input_id)
        targets.append(target)
    input_ids = torch.tensor(input_ids, dtype=torch.long)
    targets = torch.tensor(targets, dtype=torch.long)
    # print(f"input_ids为: {input_ids}")
    # print("input_ids.shape:",input_ids.shape)
    # print(f"targets为: {targets}")
...
# def preprocess_qwen_vla_tmp(sources, tokenizer: transformers.PreTrainedTokenizer, action_tokenizer: ActionTokenizer, has_image: bool = False, has_embody: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
#     # roles = {"human": "<|im_start|>user", "gpt": "<|im_start|>assistant"}
#     system_message="A chat between a curious human and an artificial intelligence robot. The robot provides actions to follow out the user's instructions."
#     roles = {"human": "user", "gpt": "assistant"}
#     # print(f"sources为: {sources}")
...
def format_source_data(sources, conv, has_embody, action_tokenizer):
    ...
    for i, source in enumerate(sources):
        ...
        conversations.append(conv.get_prompt())
    # print("在这里，conversations为", conversations)
    return conversations, real_action_token, real_obs_token

def mask_qwen_labels(input_ids):

    labels = torch.full_like(input_ids, IGNORE_INDEX)  # 先全部填充为 IGNORE_INDEX
    index_25 = (input_ids == 25).nonzero(as_tuple=True)[1]
    if len(index_25) > 0:
        last_25_index = index_25[-1].item()  # 取最后一个 25 的索引
    else:
        last_25_index = None  # 没有 25
    # print("最后一个 25 的索引:", last_25_index)
    
    # # 找到最后一个 25 的索引
    # last_25_index = (input_ids == 25).nonzero(as_tuple=True)[1][-1]
    
    # 保留 25 之后的attention mask
    # print("打印这里看看",input_ids)
...
def preprocess_action(
    ...
    """
    # print(f"conversation_lib.default_conversation为: {conversation_lib.default_conversation}")
    # print(f"default_conversation.version为: {conversation_lib.default_conversation.version}")
    if conversation_lib.default_conversation.version.startswith("qwen"):
        # print("use preprocess_qwen_vla")
        return preprocess_qwen_vla(sources, tokenizer, action_tokenizer, has_image=has_image, has_embody=has_embody)#our rossvla using
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        if overwrite_image_aspect_ratio is not None:
            ...
            print("after overwrite_image_aspect_ratio:",image_aspect_ratio.shape)
        if image_aspect_ratio == "highres":
            image = process_highres_image(image, self.data_args.image_processor, self.data_args.image_grid_pinpoints)
            print("after process_highres_image:",image.shape)
        elif image_aspect_ratio == "anyres" or "anyres_max" in image_aspect_ratio:
            ##use anyres to process the image
            image = process_anyres_image(image, self.data_args.image_processor, self.data_args.image_grid_pinpoints)
            # print("after process_anyres_image:",image.shape)
        elif image_aspect_ratio == "crop_split":
            image = process_highres_image_crop_split(image, self.data_args)
            print("after process_highres_image_crop_split:",image.shape)
            ...
            print("after expand2square:",image.shape)
        else:
            image = processor.preprocess(image, return_tensors="pt")["pixel_values"][0]
            # print("after processor.preprocess:",image.shape)
        ...
        if "image" in sources[0]:
            ...
            if type(image_file) is list:
                ...
                if len(image_file) > 1:
                    ...
                    image = [[im[0], im[1], "image"] for im in image]#[[(2*3*384*384, (384, 384), "image")],[(2*3*384*384, (384, 384), "image")]]
            else:
                image = [self.process_image(image_file)] #[(2*3*384*384, (384, 384), "image")]  单张图[(3*384*384, (384, 384), "image")]
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        # print("use DataCollatorForSupervisedDataset")
        ...
        if "image" in instances[0]:
            ...
            # else:
            batch["images"] = images#16*2*3*384*384
        # print("batch['images'][0].shape:",batch["images"][0].shape)
        # print("len(batch['images']):",len(batch["images"]))
...
def train(attn_implementation=None):
    ...
    # print("lm_head is nn.Module:", isinstance(model.lm_head, torch.nn.Module))
    # print("model.lm_head",model.lm_head)
    # # print("lm_head has parameters:", list(model.lm_head.parameters()) if hasattr(model.lm_head, 'parameters') else '❌ 无 parameters')
    # #打印可训练的模块
    # for name, param in model.named_parameters():
    ...
    # if training_args.local_rank in [1, -1]:
    #     run_llm_robot_server_from_train(model, tokenizer, data_args.image_processor) 

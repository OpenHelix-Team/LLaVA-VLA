...
from llava.model.action_model import create_diffusion
from llava.model.action_model import gaussian_diffusion as gd
import torch
...
class ActionModel(nn.Module):
    ...
    def loss(self, x, z):#z的shape需要为: (B, 1, D_h=4096), x的shape需要为: (B, T=16, D_a=7)
        # sample random noise and timestep
        noise = torch.randn_like(x) # [B, T, D_a]
        timestep = torch.randint(0, self.diffusion.num_timesteps, (x.size(0),), device= x.device)

        # sample x_t from x
        x_t = self.diffusion.q_sample(x, timestep, noise).to(z.dtype)#调度 生成加噪后某个时间步的噪声

        # predict noise from x_t
        # print("x_t.dtype:",x_t.dtype)
        # print("timestep.dtype:",timestep.dtype)
        # print("z.dtype:",z.dtype)
        noise_pred = self.net(x_t, timestep, z)#采样 在z的条件下生成操作
